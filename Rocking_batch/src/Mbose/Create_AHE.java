...
//		WebDriver driver = new FirefoxDriver();
		...
		driver.get("http://10.100.10.10:85/admin/login.aspx");
...
//		for (int r = 0; r < 100;r++) {
		...
		driver.findElement(By.xpath("//div[text()='Asst. Head Examiner']")).click();
		
		int x = 0;
		for (int r = 0; r < 50; r++) {
			String HE = Create_AHE_Generic.ddt("Sheet1", x, 0); // (row,cell)
			String F_name = Create_AHE_Generic.ddt("Sheet1", x, 1);
			String L_name = Create_AHE_Generic.ddt("Sheet1", x, 2);
			String ph = Create_AHE_Generic.ddt("Sheet1", x, 3);
			
			driver.findElement(By.xpath("//a[@href='create-asst-headexaminer.aspx']")).click();
			driver.findElement(By.xpath("//span[@class='select2-selection__arrow']")).click();
			driver.findElement(By.className("select2-search__field")).sendKeys(HE, Keys.ENTER);
